...
- Дефолти зі спеку агента використовуються лише як локальні значення (наприклад, `stop_agent_execution=false`); вони перекриваються, якщо у вхідних змінних передано актуальні host/model/temperature та інші LLM-опції.
