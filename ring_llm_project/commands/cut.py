...
class CutCommand:
    """
    CUT: copy extracted text to clipboard (overwrite), then delete it (exclusive).
    ...
    def prompt_help(self) -> str:
        return (
            "CUT — copy text between markers to clipboard (overwrite) then delete it (exclusive).\n"
            "Syntax:\n"
            "<CMD>\n"
            "id: CUT\n"
            "start: <start_marker>\n"
    ...
    def execute(self, memory: Memory, args: Dict[str, Any]) -> Memory:
        ...
        if not isinstance(start, str) or not start:
            raise CommandError("CUT: missing/invalid 'start'")
        if not isinstance(end, str) or not end:
            raise CommandError("CUT: missing/invalid 'end'")
        if start == end:
            raise CommandError("CUT: start and end cannot be identical")
        ...
        i = doc.find(start)
        if i < 0:
            raise CommandError("CUT: start marker not found")
        j = doc.find(end, i + len(start))
        if j < 0:
            raise CommandError("CUT: end marker not found after start")

        a = i + len(start)
        b = j
        if a > b:
            raise CommandError("CUT: invalid marker order")

        if _overlaps_any(a, b, prot):
            raise CommandError("CUT: target overlaps protected (service) region")

        extracted = doc[a:b]

        memory.clipboard_set(extracted, {
            "ts": _now_utc_iso(),
            "command": "CUT",
